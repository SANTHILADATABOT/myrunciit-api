	...
	function ajax_set_full(type,title,noty,form_id,id){
		...
		const path = base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id;
		console.log('load path', path); 
		ajax_load(path,'list','form');
		sound('ajax_load');
	}

	function ajax_set_full_sales_orderdetail(type,title,noty,form_id,id){
		//full_form(title,noty,form_id);
		const path = base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id;
		console.log('load path', path); 
		ajax_load(path,'order_detail_div','form');
		sound('ajax_load');
	}
    ...
    $('#fol').on('click','.submitter', function(){
        ...
        form.find(".required").each(function(){
            ...
            if(here.val() == ''){
                ...
                //$('html, body').animate({ scrollTop: $("#scroll").offset().top - topp }, 500);
                can = 'no';
            }
			...
			if($('#prod').length){
				$(".sw1").each(function(){
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  const loadUrl = base_url+'index.php/'+user_type+'/'+module+'/product_publish_set/'+$(this).data('id')+'/'+changeCheckbox.checked;
					  console.log('load url', loadUrl);
					  ajax_load(loadUrl,'prod','others');
					  if(changeCheckbox.checked == true){
						console.log('testing checked', changeCheckbox.checked);
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
							message : ppus,
				...
				$(".sw20").each(function(){
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/prod_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'prod','others');
					  if(changeCheckbox.checked == true){
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
				...
				$(".sw1").each(function(){
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/bundle_publish_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'bund','others');
					  if(changeCheckbox.checked == true){
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
				...
				$(".sw2").each(function(){
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/bundle_featured_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'bund','others');
					  if(changeCheckbox.checked == true){
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
				...
				$(".sw3").each(function(){
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/bundle_deal_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'bund','others');
					  if(changeCheckbox.checked == true){
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
	...
	function convertHTMLtoPDF(from_id,name,o)
	{
	 
	const { jsPDF } = window.jspdf;
  
	var doc = new jsPDF('p', 'mm', [1300,1900]);
	// var doc = new jsPDF(o, 'pt', 'letter');
	var pdfjs = document.querySelector('#'+from_id);
	...
	function export_it(type,ignore){
		...
		if(type == 'pdf'){
			...
			convertHTMLtoPDF('export-div',name,o,w);
		} else {
	...
	function delete_all(msg){
		...
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load(base_url+'index.php/'+user_type+'/'+module+'/delete/');
				$.activeitNoty({
					type: 'danger',
					icon : 'fa fa-check',
					message : dss,
