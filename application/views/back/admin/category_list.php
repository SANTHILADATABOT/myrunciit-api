<?php
$edit_rights=$user_rights_22['edit_rights'];
$delete_rights=$user_rights_22['delete_rights'];
?>
			...
			<tr>
				...
				<td>
				   ...
				   <td>
					...
					if($row['category_banner']!=""){
						if(file_exists('uploads/category_image/'.$row['category_banner'])){
							
					?>
					<img class="img-md" src="<?php echo base_url(); ?>uploads/category_image/<?php echo $row['category_banner']; ?>" height="100px" />  
					<?php
						} } else {
					?>
					<img class="img-md" src="<?php echo base_url(); ?>uploads/category_image/default.jpg" height="100px" />
				...
				<td>
					<input class='aiz_switchery' type="checkbox" data-set='suspend_set' data-id="<?php echo $row['category_id']; ?>" data-tm='<?php echo translate('Category Enabled'); ?>' data-fm='<?php echo translate('Category Suspended'); ?>' <?php if($row['status'] == 'ok'){ ?>checked<?php } ?> />
				</td>
               	
				<td class="text-right">
               
					<a class="btn btn-primary  btn-xs btn-labeled fa fa-puzzle-piece" data-toggle="tooltip" 
                    	onclick="ajax_modal('duplicate','<?php echo translate('duplicate_category_(_physical_product_)'); ?>','<?php echo translate('successfully_duplicated!'); ?>','category_duplicate','<?php echo $row['category_id']; ?>')" data-original-title="Duplicate" data-container="body">
  ...
  function set_switchery1(){
		$(".aiz_switchery").each(function(){
			...
			changeCheckbox.onchange = function() {
				$.ajax({url: base_url+'index.php/admin/category/'+$(this).data('set')+'/'+$(this).data('id')+'/'+changeCheckbox.checked, 
