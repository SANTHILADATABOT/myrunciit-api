			...
			<div class="col-md-12 col-lg-12">
				<div class="panel panel-bordered">
					...
					<div class="col-md-4 col-lg-4"></div>
                        ...
                        <select name="salesType" id="salesType" class="form-control" style="float:left;margin:15px;">
					...
					<div class="panel-body">
						...
						<div id="chartdivpick" style="width: 100%; height: 300px;"></div>
						<div id="chartdivdel" style="width: 100%; height: 300px;"></div>
					</div>
				</div>
			</div>
...
<script>
	...
	var chartData1 = [
    	...
    	var chartRate1 = [
		<?php
		$ratings=$this->db->query("select p.title,sum(rp.rating)/count(rp.rating) as AverageRating from review_product rp,product p where p.product_id=rp.product_id  GROUP By rp.product_id ORDER BY AverageRating DESC Limit 10")->result_array();
...
var chartData_pick = [
		...
		$this->db->where('sale_datetime BETWEEN "' . $c1time1 . '" AND "' . $ctime1 . '"');
		$this->db->where('store_id',$row_u1['vendor_id']);
		$this->db->where('order_type','pickup');
		$stock1 = $this->db->get('sale')->num_rows();
