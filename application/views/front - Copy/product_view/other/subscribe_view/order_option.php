...
?>
    ...
    <div class="col-md-8">
                <div>
                    <input type="hidden" value="<?php echo $this->session->userdata('user_id'); ?>" id="uid" name="uid">
                  <p><label>Choose package:</label> 
                  <select id="subscribe_package" name="subscribe_package" class="select" onchange="load_days(this.value);">
                      <option value="allday">Daily</option>
                      <option value="weekday">Week Days</option>
                      <option value="weekend">Week End</option>
                      <option value="customday">Custom Days</option>
                  </select>  
                  </p>  
                  
                </div>
                <div class="disabled hidden" id="packdays">
                    
                    <input type="checkbox" value="ok" id="mon" name="mon">mon
                     <input type="checkbox" value="ok" id="tue" name="tue">tue
                     <input type="checkbox" value="ok" id="wed" name="wed">wed
                     <input type="checkbox" value="ok" id="thu" name="thu">thu
                     <input type="checkbox" value="ok" id="fri" name="fri">fri
                     <input type="checkbox" value="ok" id="sat" name="sat">sat
                     <input type="checkbox" value="ok" id="sun" name="sun">sun
                     
                </div>
                <input type="hidden" value="" id="subscribamt" name="subscribamt">
            
                <div>
                  <p><label>Recharge / Topup Deliveries:</label> 
                  <select id="subscribe_recharge" name="subscribe_recharge" class="select" onchange="subs_prod_id(this.value);">
                      <?php $del_recharge = $this->db->get('subscribe_pro')->result_array(); ?>
                      <option value="">Select </option>
                      <?php foreach($del_recharge as $del_recharges) { ?>
                      <option value="<?php echo $del_recharges['id']; ?>"><?php echo $del_recharges['delivery']; ?></option>
                      <?php } ?>
                  </select>  
                  </p>  
                  
                </div>
           <?php  $added_by = json_decode($this->db->get_where('product',array('product_id'=>$row['product_id']))->row()->added_by,true);  ?>
                <input type="hidden" value="<?php echo $added_by['type']; ?>" id="added_by" name="added_by">
                <input type="hidden" value="<?php echo $added_by['id']; ?>" id="added_by_id" name="added_by_id">
                <div>
    ...
    <div class="buttons" style="display:inline-flex;">
        <span class="btn btn-add-to cart" onclick="to_cart_subs(<?php echo $row['product_id']; ?>,event)">
            <i class="fa fa-shopping-cart"></i>
			<?php if($this->crud_model->is_added_to_cart($row['product_id'])=="yes"){ 
                echo translate('subscribed');  
                } else { 
                echo translate('subscribe');  
                } 
            ?>
        </span>
        <?php 
        ...
        ?>
        <span class="btn btn-add-to hidden <?php if($wish == 'yes'){ echo 'wished';} else{ echo 'wishlist';} ?>" onclick="to_wishlist(<?php echo $row['product_id']; ?>,event)">
        ...
        ?>
        <span class="btn btn-add-to compare btn_compare hidden"  onclick="do_compare(<?php echo $row['product_id']; ?>,event)">
...
function subs_prod_id(subs_prod_id){

     $.ajax({
			url 		: base_url+'index.php/home/subscrib_prod/'+subs_prod_id,
			type 		: 'POST',
			dataType 	: 'html',
			success: function(data) {
			    $('#subscribamt').val(data);
			}
           });
