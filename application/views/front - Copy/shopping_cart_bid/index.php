<?php
echo form_open(
    base_url() . 'index.php/home/cart_finish/go',
    array(
        'method' => 'post',
...
<script>
    ...
    function load_orders() {
        var top = Number(200);
        $('.orders').html('<div style="text-align:center;width:100%;height:' + (top * 2) + 'px; position:relative;top:' + top + 'px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $('.orders').load('<?php echo base_url(); ?>index.php/home/cart_checkout_bid/orders');
    }
    ...
    function load_address_form() {
        ...
        $('.delivery_address').load('<?php echo base_url(); ?>index.php/home/cart_checkout_bid/delivery_address',
            function() {
    ...
    function load_payments() {
        ...
        if (okay == 'yes') {
            ...
            $('.payments-options').load('<?php echo base_url(); ?>index.php/home/cart_checkout_bid/payments_options',
                function() {
                    var top_off = $('.header').height();
                    $('html, body').animate({
