...
<script>
	...
	function reload_header_cart() {
	    //console.log(base_url + "index.php/home/cart/whole_list");
	$.getJSON(base_url + "index.php/home/cart/whole_list", function(result) {
			...
			var product_id = field['id'];
			//console.log('product_id',product_id)
            var product_name = field['name'].replace(/ /g, '-'); // Replace spaces with dashes
			//console.log('product_name',field['image'])
			var product_url = base_url + "index.php/home/product_view/" + product_id + "/" + product_name;
			...
			//console.log('product_url',product_url);
                ...
                "<img class=\"media-object item-image\" src=\"" + field['image'] + "\" alt=\"\">" +
                "</a>"


					+
					" <div class=\"item_count pull-right\">" +
					"<div class=\"quantity product-quantity\">" +
					"  <span class=\"btn\" name=\"subtract\" onclick=\"decrease_val2(" + field['id'] + ");\"  >" +
					"<i class=\"fa fa-minus\"></i>" +
					"</span>" +
					"<input  type=\"text\" class=\"form-control qty quantity-field cart_quantity\" min=\"1\" max=\"10\" name=\"qty\" value=\"" + field['qty'] + "\" id=\"qtyc_" + field['id'] + "\" data-rowid=\"" + field['rowid'] + "\" onchange=\"check_ours(" + field['id'] + ");\"/>"


					+
					"<span class=\"btn\" name=\"add\" onclick=\"increase_val2(" + field['id'] + ");\" >"
					...
					"</span>" +
					"    <p class=\"item-price\">" +
					"<sup>"+ currency+" </sup>" +  (field['price'] * field['qty']).toFixed(2) + " <span class=\"remove_one\"><i class=\"fa fa-trash\"></i></span></p>"
					...
					+
					"        <h4 class=\"media-heading item-title\"><a href=\"" + product_url + "\">" + field['name'] + "</a></h4>"
			...
			$('.shopping-cart__total').html("<sup>"+currency+"</sup>" + total.toFixed(2));
		});
	}
	...
	$(document).ready(function() {
		$.ajax({
			url: '<?php echo base_url(); ?>index.php/home/surfer_info'
		});
	});
</script>
...
console.log(option);
		$.ajax({
			url: base_url + 'index.php/home/cart/add/' + product + '/pp',
			type: 'POST', // form submit method get/post
			dataType: 'html', // request type html/json/xml
			data: option, // serialize form data 
			cache: false,
			...
			success: function(data) {
				...
				if (data == ' added') {
					reload_header_cart();
					//notify(product_added,'success','top','right');
					button.closest('.buttons').parent().append("<p class='notify_new '>Added to Cart</p>");
					setTimeout(function() {
						button.closest('.buttons').parent().find('p.notify_new').remove();
					}, 2000)
					button.closest('.cart').find('.response1').addClass('green');
					button.closest('.cart').find('.response1').html("Added To cart");
					setTimeout(function() {
						button.closest('.cart').find('.response1').html("");
						button.closest('.cart').find('.response1').removeClass("green");
					}, 2000)
					//sound('successful_cart');
				} else if (data == ' shortage') {
					//notify(quantity_exceeds,'warning','top','right');
					button.closest('.cart').find('.response1').addClass('red');
					button.closest('.cart').find('.response1').html(quantity_exceeds);
					setTimeout(function() {
						button.closest('.cart').find('.response1').html("");
						button.closest('.cart').find('.response1').removeClass("red");
					}, 2000)
					//sound('cart_shortage');
				} else if (data == ' already') {
					//notify(product_already,'warning','top','right');
					button.closest('.cart').find('.response1').addClass('red');
					button.closest('.cart').find('.response1').html("Already in Cart");
					setTimeout(function() {
						button.closest('.cart').find('.response1').html("");
						button.closest('.cart').find('.response1').removeClass("red");
					}, 2000)
	...
	function openSuadModal(e) {
		e.preventDefault();
		normalProcess = true;
		$('#saudaModal').modal('show');
		console.log('session data <?= $this->session->userdata('pickup') ?>');
		console.log('selected store id', localStorage.getItem('selectedStoreId'));
	}
	function checkMaxOrderIsInTimeSlot() {
	    return new Promise(function(resolve,reject){
	        console.log('checking');
    		let maxOrderNotExceed = true;
    		const pickupDetail = JSON.parse(localStorage.getItem('pickup_detail'));
    		
    		if (pickupDetail) {
    			console.log('pickupDetail', pickupDetail);
    			$.post("<?php echo base_url() . 'home/checkMaxOrderInTimeSlot/'; ?>", {
    				max_order: pickupDetail.max_order,
    				pickup_slot: pickupDetail.pickup_slot,
    				pickup_date: pickupDetail.pickup_date
    			}, function(data) {
    				console.log(data);
    				if(data == 1) resolve(true);
    				if(data == -1) resolve(false);
	...
	function showPopup(e, productId, givenstoreId) {
		e.preventDefault();
		normalProcess = false;
		$('#availableDaysSection').hide();
		$('#slotSection').hide();
		localStorage.setItem('selectedProductId', productId);
		$('#popup-7').find('.closeModal').trigger('click');
		console.log('product id', productId);
		var storeId = localStorage.getItem('selectedStoreId') ? localStorage.getItem('selectedStoreId') : givenstoreId;
		console.log('store id', storeId);
		const transportType = localStorage.getItem('transportType') ? localStorage.getItem('transportType') : '';
		zipCode = "<?= $this->session->userdata('user_zips')?>";
		if (transportType == 'delivery' && zipCode) {
			if (storeId != '' && storeId) {
    			product_to_cart_1(productId, 'pro_order');
    		} else {
    			$('#saudaModal').modal('show');
    		}
		} else {
			...
			checkMaxOrderIsInTimeSlot().then(function(data){
				...
				if (data == false) {
					alert('maximum order limit exits in this slot');
					return;
				}
				if (storeId != '' && storeId) {
					product_to_cart_1(productId, 'pro_order');
	...
	function product_to_cart_1(product, divId) {
		var formData = $('#' + divId).serialize();
		formData+="&pickup_detail="+localStorage.getItem('pickupDetail');
		$.ajax({
			url: base_url + 'index.php/home/cart/add/' + product + '/' + 'other',
			type: 'POST',
			data: formData,
			success: function(data) {
	...
	function sendData(id) {
		  var quantity_sent=document.getElementById('qtyh_'+id).value;
		  console.log(quantity_sent);
          $.ajax({
				url: base_url+'index.php/home/quantity_value/'+quantity_sent,
	...
	function product_to_cart(product, divId) {
	   var formData = new FormData($('#' + divId)[0]);
		formData.append('pickup_detail', localStorage.getItem('pickupDetail'));
...
// 		console.log(formData);
	...
	function to_cart_bid(id, e) {

		var bid_amt = $('#bid_amt').val();
		var uid = $('#uid').val();
		var uname = $('#uname').val();
		var product = id;
		e = e || window.event;
		...
		$.ajax({
			url: base_url + 'index.php/home/cart/add_bid/' + product + '/' + type + '/' + bid_amt + '/' + uid + '/' + uname,
			type: 'POST',
			dataType: 'html',
			data: option,
			cache: false,
			...
			success: function(data) {
				...
				if (data == ' added') {
					...
					notify(product_added, 'success', 'bottom', 'right');
					//sound('successful_cart');
					window.location.href = base_url + 'index.php/home/cart_checkout_bid';
					//location.window.href = base_url+'index.php/home/cart_checkout';
	...
	function to_cart_subs(id, e) {
		var subscribe_package = $('#subscribe_package').val();
		var mon = $('#mon').val();
		var tue = $('#tue').val();
		var wed = $('#wed').val();
		var thu = $('#thu').val();
		...
		var added_by_id = $('#added_by_id').val();
		var subscribamt = $('#subscribamt').val();
		var uid = $('#uid').val();
		var subscribe_recharge = $('#subscribe_recharge').val();
		...
		$.ajax({
			url: base_url + 'index.php/home/cart/add_subs/' + product + '/' + type,
			type: 'POST',
			dataType: 'html',
			data: option,
			cache: false,
			...
			sat: sat,
			sun: sun,
			subscribe_recharge: subscribe_recharge,
	...
	$(document).ready(function() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data == 'done') {
						...
						notify('Review Successfully', 'success', 'bottom', 'right');
						setTimeout(function() {
							location.replace('<?php echo base_url(); ?>index.php/home/profile');
						}, 2000)
		...
		$("body").on('click', '.return_order', function() {
			var here = $(this); // alert div for show alert message
			var text = here.html(); // alert div for show alert message
			var form = here.closest('form');
			var logging = here.data('ing');
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data == 'done') {
						...
						notify('Return Order Placed', 'success', 'bottom', 'right');
						setTimeout(function() {
							location.replace('<?php echo base_url(); ?>index.php/home/profile');
						}, 2000)
	...
	$("body").on('click', '.address_btn', function() {
		...
		$.ajax({
			...
			beforeSend: function() {
				...
				here.html('SUBMIT') // change submit button text
