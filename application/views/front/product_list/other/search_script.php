...
<?php
	...
	$this->db->select('v.vendor_id,p.store_id,p.category,p.brand');
	$this->db->from('vendor as v');
	$this->db->join('product as p', 'p.store_id = v.vendor_id');
	$this->db->where('v.vendor_id',$vendorid_get);
	$val= $this->db->get()->result_array();
	$cat_values = array();
	 //echo $this->db->last_query();
	foreach($val as $result)
	{
		$get_brand =$result['brand'];
		if (!in_array($get_brand, $cat_values)) {
		 $this->db->where('brand_id',$get_brand);
		 $all_brands = $this->db->get('brand')->result_array();
		// echo $this->db->last_query();
		foreach($all_brands as $row)
		{
			$brand_value =$row['brand_id'];?>
		<input type="hidden" name="brand_id" id="brand_id" value="<?php echo $brand_value; ?>">

	<?php	
	}
		$cat_values[] = $get_brand;
	}
}
?>
...
<script>
	...
	$(document).ready(function() {
		...
		setTimeout(function() {
			$(".search_cat_click[data-cat='" + cur_category + "']").click();
		}, 300);
	...
	function set_brands(brands) {
		...
		for (var i = 0, len = brands.length; i < len; i++) {
			...
			if (brand.length == 2) {
				...
				if (brandIds.indexOf(brand[0]) !== -1) {					
					select_brand_options = select_brand_options +'<option value="'+brand[0]+'" '+selected+' >'+brand[1]+'</option>'
				}	
			}
		}
	...
	function do_product_search(page) {
		...
		$('#sub_categoryaa').val($('.search_sub:checked').map(function() {
			return this.value;
		}).get().join(','));
