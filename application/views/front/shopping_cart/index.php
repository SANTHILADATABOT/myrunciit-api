...
<?php $vendorId = $this->session->userdata('vendorid');
        if($vendorId){
            ...
            // echo "else";
            $vendorId = $this->crud_model->defVenId();
        } ?>
<input type="hidden" name = "currstoreid" id="venId" value = "<?php echo $vendorId; ?>">
    <div class="container box_shadow">
        ...
        <div class="col-md-9">
            ...
            <input type="hidden" id="lalamove_res" name="lalamove_res" value="" />
            ...
            <div class="shopping-cart" style="background: #f5f8fa;">
                <table>
                    ...
                    <tr class="coupon_disp">
                        ...
                        <input type="text" name="total_dis" id="total_dis" value="" readonly style="text-align: right;" class="no-border">                         
                        </td>
                    </tr>
                    <tr class="coupon_disp">
                        <td style="text-align:left;font-weight:600;"><?php echo translate('delivery_estimate'); ?></td>
                        <td style="text-align:right;" ><span id="priceBreakDownTotal"><?php echo currency(); ?>0.00</span></td>
                    </tr>
                ...
                <?php //if ($this->cart->total_discount() <= 0 && $this->cart->get_coupon() == 0) { ?>
                    <h5>
                    ...
                    <div class="wells availableCouponSection">
                        ...
                        foreach($allCoupons as $row){
                            echo '<li><h4>' ."Coupon Code: ".$row['code'].'</h4></li>';
                        }
            ...
            if($(this).is(':checked')){
                ...
                $.post(url,{
                    ...
                    if(res){
                        ...
                        var price_breakdown=(jsonData.priceBreakdown.total!='')?parseFloat(jsonData.priceBreakdown.total):0;
                        priceBreakDownTotal.text(price_breakdown.toFixed(2));
...
<?php if($ipay88_alert=="yes"){ ?>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
$(document).ready(function(){
	Swal.fire(
		'<?php echo translate('incomplete_payment');?>',
		'<h4><?php echo translate('payment_canceled');?></h4>',
		'error'
	);
});
</script>
<?php } ?>
<?php if($reward_balance_alert=="yes"){ ?>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
$(document).ready(function(){
	Swal.fire(
		'<?php echo translate('insufficient_balance_in_rewards');?>',
		'<h4><?php echo translate('try_other_payment_methods');?></h4>',
		'error'
	);
});
</script>
