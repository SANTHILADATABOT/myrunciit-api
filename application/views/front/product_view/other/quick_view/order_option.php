...
?>
    <div class="order">	
        <div class="buttons">
        <input type="hidden" id="displayAmt_price" name="displayAmt_price" value="" />
        ...
        <input type="hidden" id="product_id" name="product_id" value="<?php echo $row['product_id']; ?>" />
         ...
         <!-- value="<php echo "AA".$title.'^'."BB".$op;?>"  -->
            
                      <input type="radio" onclick="return  pricevariant();" onchange="getSelectedValue();" class="optional"  name="<?php echo $name;?>" 
                      <?php if(isset($otherOptionss[str_replace(" ",'_',$title)]) && $otherOptionss[str_replace(" ",'_',$title)]==$op ) { ?> checked="checked" <?php } 
                      elseif($i==2) { ?> checked="checked" <?php } ?> value="<?php echo $op;?>" 
                      <?php if($this->crud_model->is_added_to_cart($row['product_id'], 'option', $name) == $op){ echo 'checked'; } ?> id="<?php echo $name.'_'.$i; ?>">
            ...
            var selectedValue = document.querySelector('input[name="choice_1"]:checked').value;
            document.getElementById('selectedValueInput').value = selectedValue;
            console.log('ABC---',selectedValue); // You can use this value as needed
            }

				function pricevariant(radio_val)
				{
...
<div class="buttons">
    <div id="share"></div>
    <a href="https://api.whatsapp.com/send?text=www.google.com" target="_blank" data-action="share/whatsapp/share"><img src="<?php echo base_url(); ?>uploads/whatsapp.png" width="32px;" style="margin-left: 110px;margin-top: -60px;" ></a>
</div>
...
<div class="modal fade view_in_air" id="msv" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
            <div class="modal-body">
            	<img src="" class="col-md-12 col-xs-12 col-sm-12">
            </div>
