			...
			if(count($orders)>0)
			{
				foreach ($orders as $row1)
				{
					...
					if($row1['order_trackment']==1) {
						if($row1['return_status']==1) { $row1['return_status_1']="Request"; }
						elseif($row1['return_status']==2) { $row1['return_status_1']="Acccepted"; }
						elseif($row1['return_status']==3) { $row1['return_status_1']="Reject"; }
					}
			   ...
			   // $info = json_decode($details['shipping_address'],true);
			    ...
			    foreach ($product_details as $row1) 
			    {
				   ...
				   { 
                    ...
                    if($row1['applied_gst']=='outer_country') 
					{ $sc1['shiiping_tax_rate'] = $shipping_tax_rate."%"; } else { $sc1['shiiping_tax_rate'] = ($shipping_tax_rate / 2).'%<br>'.($shipping_tax_rate / 2).'%'; }
					
					if($row1['applied_gst']=='outer_country') {  $sc1['shiiping_tax_amount'] =$shipping_tax_amount; }
                                    else {  $sc1['shiiping_tax_amount'] =($shipping_tax_amount / 2).'<br>'.currency().($shipping_tax_amount / 2); }
	...
	{	$where = '(status="success" or status = "admin_pending" or status = "rejected" or status = "cancelled")';
		...
		$orders = $this->db->get('sale')->result_array();
			//	echo $this->db->last_query();
				...
				{ 
					if($sc['refund_status']=='1'){  $status="Refunded"; }
...
//print_r($row); exit;
	...
	function product_search($para1='')
	{
		...
		if($para1!='')
		{
			...
			$productID=''; $subCatID=''; $CatID=''; $brandId=''; 
...
//$data['invoice_id'] = $invoice_id;
			...
			else
			{
					...
					if($datas['payment_type']=='wallet')
					{
						...
						$sale_code=$data['sale_code'] = date('Ym', $data['sale_datetime']) . $sale_id; exit;
						$data['delivery_status'] = json_encode($delivery_status);
						$data['payment_status'] = json_encode($payment_status);
...
require_once(APPPATH . 'libraries/ipay88/final_app.php');
	...
	function replaceproducts_add()
	{
		...
		else
		{
    		...
    		$data['added_by_id'] 		= $more_inof['id'];		
    		$data['order_id']           = $datas['order_id']; 
    		$sale_infor=$this->db->get_where('sale',array('sale_code'=>$data['order_id']))->result_array();					$sale_infor=$sale_infor[0];
    		$data['shipping']           = $sale_infor['shipping'];
    		$data['payment_method']     = $sale_infor['payment_type'];
    		$data['ordered_date'] 		= $sale_infor['created_datetime'];
    		$data['qty'] 		        = $datas['qty'];
